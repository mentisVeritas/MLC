...
WITH product_sales AS (SELECT p.category_id,
                              p.product_id,
     ...
     ranked AS (SELECT *,
                       RANK() OVER (PARTITION BY category_id ORDER BY total_sold DESC) rnk
                FROM product_sales)
SELECT *
FROM ranked
WHERE rnk = 1;
...
WITH brand_sales AS (SELECT b.brand_id,
                            SUM(oi.quantity) AS total_sold
                     FROM order_items oi
                              JOIN products p ON oi.product_id = p.product_id
...
WITH store_sales AS (SELECT o.store_id,
                            oi.quantity
                     FROM orders o
...
WITH order_size AS (SELECT o.order_id,
                           o.customer_id,
...
SELECT brand_id,
       product_name,
       list_price,
       ROW_NUMBER() OVER (PARTITION BY brand_id ORDER BY list_price, product_name) AS product_num
FROM products
ORDER BY brand_id, list_price, product_name;

-- For each category, number products alphabetically
SELECT category_id, product_name, ROW_NUMBER() OVER (PARTITION BY category_id ORDER BY product_name) AS product_num
FROM products;

-- Rank stores by total stock
SELECT store_id, SUM(quantity) AS total, RANK() OVER (ORDER BY SUM(quantity) DESC) AS rank_by_total
FROM stocks
GROUP BY store_id;
...
SELECT brand_id,
       ...
       RANK() OVER (ORDER BY AVG(list_price)) AS rank_by_avg_price
FROM products
GROUP BY brand_id;
...
SELECT product_id,
       ...
       brand_id,
       list_price,
       ROUND(AVG(list_price) OVER (PARTITION BY brand_id), 2) AS avg_brand_price
FROM products
ORDER BY product_id;

-- Show each order with the average order quantity of that customer
WITH order_qty AS (SELECT o.order_id,
                          o.customer_id,
...
SELECT order_id,
       ...
       AVG(order_quantity) OVER (
           ...
           ) AS avg_order_quantity_per_customer
FROM order_qty
ORDER BY customer_id, order_id;


-- Show each stock row with the maximum stock of that product
SELECT *, MAX(quantity) OVER (PARTITION BY product_id) AS max_stock
FROM stocks
ORDER BY product_id;
...
SELECT product_id,
       ...
       brand_id,
       list_price,
       LAG(list_price) OVER (PARTITION BY brand_id ORDER BY list_price) prev_product_price
FROM products;
...
SELECT product_id,
       product_name,
       list_price,
       LEAD(list_price) OVER (ORDER BY list_price, product_id) next_higher_price
FROM products
ORDER BY list_price;

-- Find customers whose latest order is bigger than their previous one
WITH order_totals AS (SELECT o.customer_id,
                             o.order_id,
                             o.order_date,
                             SUM(oi.quantity) AS order_quantity
                      FROM orders o
     ...
     orders_with_prev AS (SELECT customer_id,
                                 order_id,
                                 order_date,
                                 order_quantity,
                                 LAG(order_quantity) OVER (PARTITION BY customer_id ORDER BY order_date) AS prev_order_quantity
                          FROM order_totals)
SELECT *
FROM orders_with_prev
WHERE prev_order_quantity IS NOT NULL
  AND order_quantity > prev_order_quantity;

-- Find brands where the most expensive product is 2× more than average
WITH brand AS (SELECT brand_id,
